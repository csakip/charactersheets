...
export default function NewCharacterDialog({
  ...
  visible: boolean;
  onHide: () => void;
  onSave: ({ playerName, rollChecked, system, type }: { playerName: string; rollChecked?: boolean; system?: string; type?: string }) => void;
  system?: string;
}) {
    ...
    <Dialog
      ...
      footer={
        <div>
          ...
          <Button label='Létrehozás' icon='pi pi-check' onClick={handleSave} disabled={!value.trim()} />
        </div>
      }>
      <div className='flex flex-column gap-2'>
        ...
        <div className='flex flex-column gap-2'>
          ...
          <InputText id='value' value={value} onChange={(e) => setValue(e.target.value)} onKeyDown={(e) => e.key === "Enter" && handleSave()} />
          {selectedSystem === "wodu" && (
            <div className='mt-1'>
              <Checkbox onChange={(e) => setRollChecked(e.checked)} checked={rollChecked} inputId='rollChecked' />
          ...
          {selectedSystem === "hidestarwars" && (
            <div className='flex flex-column gap-2'>
              ...
              <Dropdown className='w-full' value={type} options={types} onChange={(e) => setType(e.value)} placeholder='Karakterlap tipus' />
            </div>
          )}
        </div>
      </div>
